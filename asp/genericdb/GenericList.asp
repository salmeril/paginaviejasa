...
' Revision History:
' 15 Jul 1999 - Unicode memo fields now displayed with <br>'s for CR's too.
' 14 Jul 1999 - Added Response.Clear before Redirect for boneheaded MSIE browsers
' 12 Jul 1999 - Fixed dbTotalFields support, Bottom left-most column is always "Totals:"
'  8 Jul 1999 - Fixed dbEMailFor support - was linking wrong field
'  6 Jul 1999 - Fixed dbEMailFor support
' 30 Jun 1999 - Language support
'				Cleaned up currency, URL and MAILTO display
'				Session("dbSearchPos") = "TOP" will display the search box at the top of the page.
'				Session("dbExitPageText") = "My Exit Link Text Here" (alternative to Back)
'				Session("dbHidePageNumbers") = 1 option to hide page numbers for long result sets
' 29 Jun 1999 - Support for HAVING clause (dbHaving)
' 28 Jun 1999 - Error handler included for bad queries
' 23 Jun 1999 - Option to strip #'s from hyperlink fields, search for *** and uncomment the code
'  9 Jun 1999 - Support for reverse ordering: use a negative OrderBy
'  1 May 1999 - dbTotalFields 
' 20 Apr 1999 - dbEditTemplate, dbBorderColor, dbMenuColor, dbFont and dbFontSize settings
' 10 Mar 1999 - Page numbers 
'  3 Mar 1999 - Search: prompt to Reset only shown if dbWhere was set in the Config File.
...
<% 		End If
		...
		&nbsp;&nbsp;|&nbsp;&nbsp;<A HREF="<%=Session("dbGenericPath")%><%=strEditor%>?CMD=NEW"><%=txtAddRecord%></A>
<% End If %>
	</TD>
</TR>
...
'						curVal = Replace(curVal,"#","")
				...
				If (aFields(x,2) = 201) OR (aFields(x,2) = 204) Then curVal = replace(curVal,chr(10),"&nbsp;<br>")
...
<FORM ACTION="GenericSearchResult.asp" METHOD="POST">
	<TR>
		<TD HEIGHT=0 BGCOLOR="WHITE" "ALIGN="LEFT">
			<FONT SIZE=2 FACE="<%=strFont%>"><%=txtSearchFor%></TD>
		<TD HEIGHT=0 BGCOLOR="WHITE" ALIGN="LEFT">
			<FONT SIZE=2 FACE="<%=strFont%>"><INPUT TYPE="Text" NAME="strSearch" SIZE=40></TD>
		<TD HEIGHT=0 BGCOLOR="WHITE" ALIGN="LEFT">
			<FONT SIZE=2 FACE="<%=strFont%>"><INPUT TYPE="Submit" NAME="Submit" VALUE="<%=txtSearchSubmit%>"></TD>
	</TR>
</FORM>
</TABLE></TD></TR></TABLE>
<p>
