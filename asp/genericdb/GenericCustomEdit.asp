...
<FORM ACTION="GenericCustomEdit.asp" METHOD=POST>
<%
Dim SkipToNext
...
While Not ts.AtEndOfStream
	...
	strLine = Replace(strLine, "</HTML>","",1,-1,1)
'	If Instr(1, strLineU, "<BODY") > 0 Then	SkipToNext = True
			...
			Else ' display the field
				...
				While Len(strLine) > 0	' parse the values 
					if UCase(Left(strLine,4)) = "NAME" Then
						strLine = Replace(strLine,"NAME","",1,-1,1)
						strLine = Replace(strLine,"=","")
						strLine = LTrim(strLine)
						strLineU = UCase("strLine")
						position = Instr(1, strLine, " ")
	...
	<INPUT TYPE="Password" NAME="<%=aFields(x,1)%>" VALUE="<%=aFields(x,4)%>" SIZE=40 MAXLENGTH="<%=aFields(x,3)%>">
<%		Else 
			Select Case aFields(x,2) 
				Case 2 ' 2-Byte Integer
%>
	<INPUT TYPE="Text" NAME="<%= aFields(x,1) %>" VALUE="<%= aFields(x,4) %>" SIZE=10 MAXLENGTH=4>
<% 				Case 3 ' 4-Byte Integer 
%>
	<INPUT TYPE="Text" NAME="<%= aFields(x,1) %>" VALUE="<%= aFields(x,4) %>" SIZE=10 MAXLENGTH=8>
...
<% 				Case 135 ' Date / Time Stamp, usually created with the Now() function
%>
	<INPUT TYPE="Text" NAME="<%= aFields(x,1) %>" VALUE="<%= aFields(x,4) %>" SIZE=40 MAXLENGTH=40>
...
<%							' LIST
		...
		<OPTION VALUE="<%= arrCombo(y) %>" <% If arrCombo(y)=aFields(x,4) Then Response.Write" SELECTED" %>> <%=arrCombo(y+1)%>
<%								Next
							End If
							' TABLE 
...
%>
	<P>
	<FONT SIZE="3"><INPUT TYPE="SUBMIT" NAME="Action" VALUE="<%=SUBMITVALUE%>"></FONT>
</FORM>
