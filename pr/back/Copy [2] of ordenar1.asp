...
rsordenes.ActiveConnection = "dsn=quotes;"
    ...
    <td align="left" valign="top"><!-- #BeginEditable "contenido" --> 
      <table width="75%" border="0" cellspacing="10" cellpadding="10">
        <tr align="center"> 
          <td> 
            <p><%
vid =(rsordenes.Fields.Item("ID").Value)

Set objMail = CreateObject("CDONTS.Newmail")
objMail.From = "system"
objMail.To = "powernatural@paradero.com"
objMail.Subject = "Nueva orden"
objMail.Body = "para ver la orden haga click aqui     http://www.nuestramedicina.com/pr/ordendetalle.asp?id=" & vid
objMail.Send
 %> 
            </p>
            ...
            <p><font face="Verdana, Arial, Helvetica, sans-serif">Total:<b> $<%=(rsordenes.Fields.Item("total").Value)%>+$10</b> 
              (gastos de envio</font></p>
            <% if (rsordenes.Fields.Item("tarjeta").Value)= "cheque" then %> 
            Envienos su cheque a<% else %><br>
            <% if (rsordenes.Fields.Item("tarjeta").Value)= "money" then %> 
            Envienos el money order a <% else %><br>
            <% if (rsordenes.Fields.Item("tarjeta").Value)= "western" then %> 
            Par mandar su pago por Western Union, haga <a href="https://www6.westernunion.com/webmt/default.asp?M27PRJ=webmt&amp;DISPATCHER=WebMT&amp;RequestID=SvrQCAMTSCR">click 
            aqui</a> <br>
            <% else %> 
            seran cargados en su tarjeta de credito<% end if%> 
            <% end if%> 
            <% end if%> 
            <p>&nbsp;</p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif">PARA CUALQUIER 
              PREGUNTA PUEDE LLAMAR AL</font></p>
            <p><font face="Verdana, Arial, Helvetica, sans-serif"> <b>1-800-522-7099</b></font></p>
